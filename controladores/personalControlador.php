...
class personalControlador extends personalModelo
{
	...
	//Controlador para agregar personal
	public function eliminar_personal_controlador()
	{
		...
		$guardarEstadop = personalModelo::eliminar_personal_modelo($dataAD);

		if ($guardarEstadop->rowCount() >= 1) {


			$consulta = mainModel::ejecutar_consulta_simple("SELECT nombre_personal FROM tpersonal WHERE idpersonal='$idpersonal'");
			foreach ($consulta as $row) {
				$nombreu = $row['nombre'];
			}
			$textoestadop = "Se dio de baja al empleado";
	...
	{
		$nombre = mainModel::limpiar_cadena($_POST['pernombre']);
		$apellido = mainModel::limpiar_cadena($_POST['perapellido']);
		$telefono = mainModel::limpiar_cadena($_POST['pertelefono']);
		$dui = mainModel::limpiar_cadena($_POST['perdui']);
